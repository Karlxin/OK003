//implements fminlbfgs,this has not been finished
...
{
	string Display = "off";
	string HessUpdate = "bfgs";
	uint8_t GoalsExactAchieve = 1;
	bool GradConstr = true;
	double TolX = 1e-6;
	double TolFun = 1e-6;
	string GradObj = "on";
	uint32_t MaxIter = 400;
	uint32_t MaxFunEvals = 0;
	double DiffMaxChange = 1e-1;
	double DiffMinChang = 1e-8;
	double rho = 0.0100;
	double sigma = 0.900;
	uint32_t tau1 = 3;
	double tau2 = 0.1;
...
pair<Mat<double>, Mat<double>> fminlbfgs(int32_t visibleSize,
	...
	Mat<double> x_init, int32_t options)
{
	...
	defaultopt.MaxFunEvals = 100 * x_init.n_elem - 1;
	...
	data.xsizes = { (uint32_t)(x_init.n_rows), (uint32_t)x_init.n_cols };
	data.numberOfVariables = x_init.n_elem;
	data.xInitial = vectorise(x_init);
	data.alpha = 1;
	...
	data.exitflag = false;
	data.nStored = 0;
	data.Hessian.zeros(data.numberOfVariables, data.numberOfVariables);

	time(&timer);
	seconds = difftime(timer, mktime(&y2k));
	data.timeTotal = seconds;
	data.timeExtern = 0;
	...
	{
		defaultopt.HessUpdate = "bfgs";
	}
	else
	{
		defaultopt.HessUpdate = "lbfgs";
	}

	bool succes;

	if (!defaultopt.HessUpdate.compare("lbfgs"))
	{
		succes = false;
		while (~succes)
		{
			try
			{
				data.deltaX.zeros(data.numberOfVariables, defaultopt.StoreN);
				data.deltaG = zeros(data.numberOfVariables, defaultopt.StoreN);
				data.saveD = zeros(data.numberOfVariables, defaultopt.StoreN);
				succes = true;
			}
			catch (int e)
			{
				...
				succes = false;
				data.deltaX = { 0 }; data.deltaG = { 0 }; data.saveD = { 0 };
				defaultopt.StoreN = defaultopt.StoreN - 1;
				if (defaultopt.StoreN < 1)
				{
					cout << "An exception occurred. Exception Nr. " << e << '\n' << endl;
				}
			}
	...
	if (!defaultopt.Display.compare("iter"))
	{
		cout << "   Iteration  Func-count   Grad-count         f(x)         Step-size " << '\n' << endl;
	}


	pair<double, Mat<double>> fval_grad_pair;

	// Calculate the initial error and gradient
	data.initialStepLength = 1;
	fval_grad_pair = UFLDL_get_Cost_Grad(x_init, visibleSize, hiddenSize,
		lambda, sparsityParam, beta, patches);

	data.gradient = fval_grad_pair.second;
	data.dir = -data.gradient;
	data.fInitial = fval_grad_pair.first;
	data.fPrimeInitial = as_scalar(data.gradient.t()*data.dir);
	data.fOld = data.fInitial;
	data.xOld = data.xInitial;
	data.gOld = data.gradient;

	double gNorm;
	gNorm = as_scalar(norm(conv_to<vec>::from(data.gradient), "inf"));  // Norm of gradient

	data.initialStepLength = min((int)(1 / gNorm), 5);
	...
	if (!defaultopt.HessUpdate.compare("bfgs"))
	{
		data.Hessian.eye(data.numberOfVariables, data.numberOfVariables);
	}
	...
	data.TolFunLnS = eps(max(1.0, abs(data.fInitial)));
	...
	if (defaultopt.Display(1) == 'p'), figure, hold on; end

		// Find a good step size in the direction of the gradient : Linesearch
		if (defaultopt.GoalsExactAchieve == 1)
			data = linesearch(funfcn, data, defaultopt);
		else
			data = linesearch_simple(funfcn, data, defaultopt);
	end

		// Make linesearch plot
		if (defaultopt.Display(1) == 'p');
	...
	for i = 1:length(alpha_test)
		[data, falpha_test(i)] = gradient_function(data.xInitial(:) + alpha_test(i)*data.dir(:), funfcn, data, defaultopt);
	end
	...
	gNorm = norm(data.gradient, Inf);  // Norm of gradient
		...
		// Set exit flags
	if (gNorm < defaultopt.TolFun), exitflag = 1; end
		if (max(abs(data.xOld - data.xInitial)) < defaultopt.TolX), exitflag = 2; end
			if (data.iteration >= defaultopt.MaxIter), exitflag = 0; end

				// Check if exitflag is set
				if (~isempty(exitflag)), break, end;

	// Update the inverse Hessian matrix
	if (defaultopt.HessUpdate(1)~= 's')
		// Do the Quasi - Neton Hessian update.
		data = updateQuasiNewtonMatrix_LBFGS(data, defaultopt);
	else
		data.dir = -data.gradient;
	end
		...
		if (call_output_function(data, defaultopt, 'iter')), exitflag = -1; end

			// Show the current iteration
			if (strcmp(defaultopt.Display(1), 'i') || strcmp(defaultopt.Display(1), 'p'))
	...
	// Call output function
	if (call_output_function(data, defaultopt, 'done')), exitflag = -1; end

		// Make exist output structure
		if (defaultopt.HessUpdate(1) == 'b'), output.algorithm = 'BroydenFletcherGoldfarbShanno (BFGS)';
	elseif(defaultopt.HessUpdate(1) == 'l'), output.algorithm = 'limited memory BFGS (L-BFGS)';
		...
		if (~strcmp(defaultopt.Display, 'off'))
						...
						case -1, message = 'Algorithm was terminated by the output function.';
							case -2, message = 'Line search cannot find an acceptable point along the current search';
								otherwise, message = 'Undefined exit code';
									...
									function stopt = call_output_function(data, defaultopt, where)
									stopt = false;
								if (~isempty(defaultopt.OutputFcn))
									output.iteration = data.iteration;
								output.funccount = data.funcCount;
								output.fval = data.fInitial;
								output.stepsize = data.alpha;
								...
								output.gradient = reshape(data.gradient, data.xsizes);
								output.searchdirection = data.dir;
								stopt = feval(defaultopt.OutputFcn, reshape(data.xInitial, data.xsizes), output, where);
								end


									function data = linesearch_simple(funfcn, data, defaultopt)
									// Find a bracket of acceptable points
									data = bracketingPhase_simple(funfcn, data, defaultopt);
								...
								// now find acceptable point within bracket
									data = sectioningPhase_simple(funfcn, data, defaultopt);
								data.exitflag = data.section_exitflag;
									...
									function data = bracketingPhase_simple(funfcn, data, defaultopt)
								...
								while (true)
									...
									if (defaultopt.GradConstr)
										[data, f_alpha] = gradient_function(data.xInitial(:) + alpha*data.dir(:), funfcn, data, defaultopt);
								fPrime_alpha = nan;
								grad = nan;
									else
										[data, f_alpha, grad] = gradient_function(data.xInitial(:) + alpha*data.dir(:), funfcn, data, defaultopt);
								...
								if (itw > (log(defaultopt.TolFun) / log(defaultopt.tau3))),
								...
								if (gstep > defaultopt.DiffMaxChange), gstep = defaultopt.DiffMaxChange; end
									if (gstep < defaultopt.DiffMinChange), gstep = defaultopt.DiffMinChange; end
										[data, f_alpha2] = gradient_function(data.xInitial(:) + (alpha + gstep)*data.dir(:), funfcn, data, defaultopt);
								[data, f_beta2] = gradient_function(data.xInitial(:) + (beta + gstep)*data.dir(:), funfcn, data, defaultopt);
								...
								data.bracket_exitflag = 2; return
									...
									if (data.funcCount >= defaultopt.MaxFunEvals), data.bracket_exitflag = 0; return; end
										end


										function data = sectioningPhase_simple(funfcn, data, defaultopt)
										// Get the brackets
										brcktEndpntA = data.a; brcktEndpntB = data.b;

								// Calculate minimum between brackets
								[alpha, f_alpha_estimated] = pickAlphaWithinInterval(brcktEndpntA, brcktEndpntB, data.a, data.b, data.f_a, data.fPrime_a, data.f_b, data.fPrime_b, defaultopt);
								...
								function data = linesearch(funfcn, data, defaultopt)

									// Find a bracket of acceptable points
									data = bracketingPhase(funfcn, data, defaultopt);

								if (data.bracket_exitflag == 2)
									// BracketingPhase found a bracket containing acceptable points;
								// now find acceptable point within bracket
									data = sectioningPhase(funfcn, data, defaultopt);
								data.exitflag = data.section_exitflag;
								else
									// Already acceptable point found or MaxFunEvals reached
									data.exitflag = data.bracket_exitflag;
									...
									function data = sectioningPhase(funfcn, data, defaultopt)
									//
									...
									while (true)
										...
										brcktEndpntA = data.a + min(defaultopt.tau2, defaultopt.sigma)*(data.b - data.a);
								brcktEndpntB = data.b - defaultopt.tau3*(data.b - data.a);
								...
								alpha = pickAlphaWithinInterval(brcktEndpntA, brcktEndpntB, data.a, data.b, data.f_a, data.fPrime_a, data.f_b, data.fPrime_b, defaultopt);
								...
								if ((f_alpha > data.fInitial + alpha*defaultopt.rho*data.fPrimeInitial) || (f_alpha >= data.f_a))
									...
									if (abs(fPrime_alpha) <= -defaultopt.sigma*data.fPrimeInitial),
										if (defaultopt.GradConstr)
								...
								if (data.b - data.a)*fPrime_alpha >= 0
									...
									if (abs(data.b - data.a) < eps), data.section_exitflag = -2; return, end
										...
										// maxFunEvals reached
										if (data.funcCount > defaultopt.MaxFunEvals), data.section_exitflag = -1; return, end
											end

											function data = bracketingPhase(funfcn, data, defaultopt)
								...
								alphaMax = (data.fminimum - data.fInitial) / (defaultopt.rho*data.fPrimeInitial);
								alphaPrev = 0;
								...
								if (f_alpha <= data.fminimum), data.bracket_exitflag = 4; return; end
									...
									if (f_alpha > (data.fInitial + alpha*defaultopt.rho*data.fPrimeInitial)) || (f_alpha >= fPrev)
								...
								brcktEndpntB = min(alphaMax, alpha + defaultopt.tau1*(alpha - alphaPrev));
								...
								alphaNew = pickAlphaWithinInterval(brcktEndpntA, brcktEndpntB, alphaPrev, alpha, fPrev, ...
									fPrimePrev, f_alpha, fPrime_alpha, defaultopt);
								alphaPrev = alpha;
								alpha = alphaNew;
									else
										alpha = alphaMax;
									...
									// maxFunEvals reached
									if (data.funcCount > defaultopt.MaxFunEvals), data.bracket_exitflag = -1; return, end
										end

										function[alpha, f_alpha] = pickAlphaWithinInterval(brcktEndpntA, brcktEndpntB, alpha1, alpha2, f1, fPrime1, f2, fPrime2, defaultopt)
								...
								plot(xPoints*(alpha2 - alpha1) + alpha1, vPoints, 'c');
									...
									function[data, fval, grad] = gradient_function(x, funfcn, data, defaultopt)
								...
								data.funcCount = data.funcCount + 1;
										...
										if (strcmp(defaultopt.GradObj, 'on'))
											timem = tic;
								...
								grad(i) = (fval_g - fval) / gstep;
									...
									function data = updateQuasiNewtonMatrix_LBFGS(data, defaultopt)
								...
								data.dir = -data.Hessian*data.gradient;
										...
										// L - BFGS with scaling as described by Nocedal
											...
											data.deltaX(:, 2 : defaultopt.StoreN) = data.deltaX(:, 1 : defaultopt.StoreN - 1); data.deltaX(:, 1) = deltaX;
								data.deltaG(:, 2 : defaultopt.StoreN) = data.deltaG(:, 1 : defaultopt.StoreN - 1); data.deltaG(:, 1) = deltaG;

								data.nStored = data.nStored + 1; if (data.nStored > defaultopt.StoreN), data.nStored = defaultopt.StoreN; end
...
//data.xInitial,fval,grad
tuple<Mat<double>, double, Mat<double>> gradient_function(Mat<double> x, int32_t visibleSize,
	int32_t hiddenSize, double lambda, double sparsityParam, double beta, Mat<double> patches,
	Mat<double> x_init, int32_t options, data_struct data,opt optim)
{
	// Call the error function for error(and gradient)
		if (nargout <3)
			timem = tic;
